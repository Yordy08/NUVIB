...
export const validateProductInput = (body: any, current?: any) => {
  ...
  const previousPrice = body?.previousPrice === '' || body?.previousPrice == null ? null : Number(body.previousPrice)
  ...
  return {
    ...
    weight: text(body?.weight, 40), dimensions: text(body?.dimensions, 100),
