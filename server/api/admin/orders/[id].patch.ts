...
export default defineEventHandler(async (event) => {
  ...
  if (body.customer) update.customer = { ...current.customer, name: typeof body.customer.name === 'string' ? body.customer.name.trim().slice(0, 100) : current.customer.name, phone: typeof body.customer.phone === 'string' ? body.customer.phone.trim().slice(0, 30) : current.customer.phone }
  if (body.delivery) update.delivery = { ...current.delivery, country: typeof body.delivery.country === 'string' ? body.delivery.country.trim().slice(0, 60) : current.delivery.country, department: typeof body.delivery.department === 'string' ? body.delivery.department.trim().slice(0, 80) : current.delivery.department, city: typeof body.delivery.city === 'string' ? body.delivery.city.trim().slice(0, 80) : current.delivery.city, address: typeof body.delivery.address === 'string' ? body.delivery.address.trim().slice(0, 180) : current.delivery.address, additional: typeof body.delivery.additional === 'string' ? body.delivery.additional.trim().slice(0, 240) : current.delivery.additional }
